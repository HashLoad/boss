...
func (c *DefaultGitClient) CloneCache(dep models.Dependency) *goGit.Repository {
	return git.CloneCache(dep)
}

// UpdateCache updates an existing cached repository.
func (c *DefaultGitClient) UpdateCache(dep models.Dependency) *goGit.Repository {
	return git.UpdateCache(dep)
}

// GetRepository returns the repository for a dependency.
func (c *DefaultGitClient) GetRepository(dep models.Dependency) *goGit.Repository {
	return git.GetRepository(dep)
}

// GetVersions returns all version tags for a repository.
func (c *DefaultGitClient) GetVersions(repository *goGit.Repository, dep models.Dependency) []*plumbing.Reference {
