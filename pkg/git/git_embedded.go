...
func (g *GitProvider) CloneCache(dep models.Dependency) *git.Repository {
	...
	if err != nil {
		...
		msg.Fatal("Error to get repository of %s: %s", dep.Repository, err)
	}
	initSubmodules(dep, repository)
	return repository
}

func (g *GitProvider) UpdateCache(dep models.Dependency) *git.Repository {
	storageCache := makeStorageCache(dep)
	...
	if err != nil && errors.Is(err, git.ErrRepositoryNotExists) {
		...
		msg.Info("Trying to recreate cache...")

		return g.CloneCache(dep)
	} else {
...
func makeStorageCache(dep models.Dependency) storage.Storer {
	...
	newStorage := filesystem.NewStorage(fs, cache.NewObjectLRUDefault())
	return newStorage
}
